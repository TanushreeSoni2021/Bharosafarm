  ...
  useEffect(() => {
    Axios.get(`http://localhost:4000/heading/${1}`)
      .then(response => {
        ...
        localStorage.setItem('myheading', JSON.stringify(response.data))
  ...
  useEffect(() => {
    Axios.get(`http://localhost:4000/about/${1}`)
      .then(response => {
        ...
        localStorage.setItem('myAbout', JSON.stringify(response.data))
  ...
  useEffect(() => {
    Axios.get(`http://localhost:4000/aboutimg`)
      .then(response => {
        ...
        localStorage.setItem('mytTooltip', JSON.stringify(response.data))
  ...
  useEffect(() => {
    Axios.get(`http://localhost:4000/aboutproduct/${1}`)
      .then(res => {
        console.log(res)
        setProduct({ val: res })
        localStorage.setItem('myPoduct', JSON.stringify(res.data))
  ...
  useEffect(() => {
    Axios.get(`http://localhost:4000/partners`)
      .then(res => {
        console.log(res)
        setPartner({ val: res })
        localStorage.setItem('myPartner', JSON.stringify(res.data))
  ...
  useEffect(() => {
    Axios.get(`http://localhost:4000/position/${1}`)
      .then(response => {
        console.log(response)
        setPos({ val: response })
        localStorage.setItem('myPosition', JSON.stringify(response.data))
      ...
      <section>
        <div className='container-fluid d-flex mt-5 justify-content-center align-items-center bg-img'>
          <div className="styling">
            <img alt='' className='row d-flex' src={myproduct?.val.data.logo} height={"80px"} width={"80px"} style={{ 'margin': '0 auto' }} /><br />
      ...
      <section>
        <div className='container d-flex justify-content-center align-items-center mt-5'>
          <div className="styling pt-5" >
            <img className='row d-flex' src={mypos?.val.data.icon} style={{ 'margin': '0 auto' }} alt='' /><br />
